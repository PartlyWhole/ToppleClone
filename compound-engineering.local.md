...
This is a **Godot 4.6 + GDScript tower building/stacking physics game** (TowerGame). All reviews
...
- **Scene safety:** `.tscn` files must NOT be structurally edited by
  agents. Read and report only. Property value changes are OK.
...
- **Physics:** Tower blocks use RigidBody2D. Stacking detection via
  Area2D overlap or contact monitoring. Gravity and physics material
  tuning lives in resources.
- **Mobile input:** Touch-only. No keyboard shortcuts for gameplay.
  UI buttons must meet minimum 48px touch target size.
